...
app = FastMCP(
    ...
    instructions=(
        ...
        "visual_inspect_slides(slides=[n]); if it reports issues, pass that "
        "same list to visual_repair_slides(slides=[n], issues=...) — the "
        "repair then skips a review you already paid for. If a repair round "
        "reports that nothing was applied, or that the findings are not "
        "going down, rebuild the slide content with the editing tools rather "
        "than calling repair again. Before exporting, run "
        "validate_presentation for structural faults and one whole-deck "
        "visual_inspect_slides for appearance. Deliver the deck with "
