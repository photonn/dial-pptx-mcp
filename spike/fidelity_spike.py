...
def compare_parts(template, output, prefix, label):
    """Compare a family of OOXML parts byte-for-byte between the two files."""
    ...
    changed = [
        n for n in t_names
        if n in o_names and read_part(template, n) != read_part(output, n)
    ]
    if changed:
        issues.append(f"{label}: CHANGED (bytes differ): {changed}")
